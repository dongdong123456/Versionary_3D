...
class Runthread(QtCore.QThread):
    ...
    # 目标检测
    def detectron(self, image, distanceData, numRows, numCols, img_bgr,frameData):
        ...
        if layer_Z > 5000:
            ...
            for box_detail in box_list:
                # 进行坐标转换
                print(box_detail)
                P_rob = box_detail[5:8]
                send_message = '001' + ',' + '%.0f' % box_detail[5] + ',' + '%.0f' % box_detail[6] + ',' + '%.0f' % box_detail[7] + ',' + '%.0f' % (box_detail[8]) + ',' + str(len(box_list)) + '\n'
                send_masage.append(send_message)
        return img_bgr, send_masage
...
class Show_logic(QMainWindow, Ui_Show):
    ...
    def model_path(self):
        ...
        if fname[0]:
            try:
                ...
                self.write_msg(msg)
                self.label_4.setStyleSheet(self.m_green_SheetStyle)
            except:
    ...
    def click_save(self):
        if self.status == True:
            ...
            img_bgr, send_masage=self.thread.detectron(b, distanceData, numRows, numCols, img_bgr,frameData)
            self.read_robot_image(img_bgr)
            self.write_msg(''.join(send_masage))
        else:
